/* Declares the iPlugWorkshop Audio Worklet Node */

class iPlugWorkshopController extends WAMController
{
  constructor (actx, options) {
    ...
    super(actx, "iPlugWorkshop", options);
  }

  static importScripts (actx) {
    var origin = "https://dwahlqvist.github.io/iplugworkshop/";

    return new Promise( (resolve) => {
      actx.audioWorklet.addModule(origin + "scripts/iPlugWorkshop-wam.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/wam-processor.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/iPlugWorkshop-awp.js").then(() => {
        resolve();
      }) }) });
    })
  }
