...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\iPlugWorkshop Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\iPlugWorkshop Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\installer\changelog.txt",
      projectpath + "\installer\known-issues.txt",
      projectpath + "\manual\iPlugWorkshop manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\iPlugWorkshop.vst3\Contents\\x86_64-win\iPlugWorkshop.vst3",
      projectpath + "\\build-win\iPlugWorkshop_x64.exe"  
    ]

  zipname = "iPlugWorkshop-v" + config['FULL_VER_STR']
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\iPlugWorkshop-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\iPlugWorkshop-app_x64.pdb"  
  ]
