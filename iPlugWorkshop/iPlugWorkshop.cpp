    ...
    : Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  ...
  mLayoutFunc = [&](IGraphics *pGraphics) {
    ...
    // pGraphics->AttachPanelBackground(COLOR_WHITE); /* TASK_01 */
    ...
    pGraphics->AttachControl(new IVLabelControl(logoArea, "iPlugWorkshop", DEFAULT_STYLE.WithDrawFrame(false).WithValueText(IText(24., "Logo"))));
...
#if IPLUG_DSP
void iPlugWorkshop::ProcessBlock(sample **inputs, sample **outputs, int nFrames)
{
...
void iPlugWorkshop::ProcessMidiMsg(const IMidiMsg &msg)
{
  mSynth.AddMidiMsgToQueue(msg);
}

void iPlugWorkshop::OnReset()
{
  mSynth.SetSampleRateAndBlockSize(GetSampleRate(), GetBlockSize());
}

void iPlugWorkshop::OnParamChange(int paramIdx)
{
  ...
  case kParamAmpAttack:
    ...
    {
      voice->mEnv.SetStageTime(ADSREnvelope<sample>::EStage::kAttack, value);
    }
    break;
  case kParamAmpDecay:
    for (auto *voice : mVoices)
    {
      voice->mEnv.SetStageTime(ADSREnvelope<sample>::EStage::kDecay, value);
    }
    break;
  case kParamAmpSustain:
    for (auto *voice : mVoices)
    {
      voice->mSustainLevel = value / 100.0;
    }
    break;
  case kParamAmpRelease:
    for (auto *voice : mVoices)
    {
      voice->mEnv.SetStageTime(ADSREnvelope<sample>::EStage::kRelease, value);
    }
    break;
  default:
    break;
